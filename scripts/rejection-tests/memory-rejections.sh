...
run_host_case() {
  bash scripts/with-build-lock.sh \
    ...
    bash -lc "make clean >/dev/null 2>&1 || true; make -B iso-test arch='${ARCH}' KFS_TEST_BAD_MEMORY=1 >/dev/null && KFS_HOST_TEST_DIRECT=1 TEST_TIMEOUT_SECS='${TIMEOUT_SECS}' TEST_FAIL_RC='${FAIL_RC}' bash scripts/rejection-tests/memory-rejections.sh '${ARCH}' '${CASE}'"
}
