...
printf '\n'
color "1;34"; printf '%s\n' "TESTS"; reset_color
run_item 1 16 "release ISO is bootable" \
...
run_item 2 16 "kmain exists + is called (M4.1)" \
  bash scripts/container.sh run -- \
    bash -lc "bash scripts/check-m4.1-kmain.sh '${ARCH}'"

run_item 3 16 "string helpers pass host tests (M5.2)" \
  bash scripts/check-m5.2-string.sh "${ARCH}"

run_item 4 16 "memory helpers pass host tests (M5.3)" \
  bash scripts/check-m5.3-memory.sh "${ARCH}"

run_item 5 16 "release IMG is bootable" \
  bash scripts/container.sh run -- \
